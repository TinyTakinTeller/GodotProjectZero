...
func _ready():
	...
	elif auto_distance_from != NodePath():
		set_physics_process(true)
	elif auto_pattern_id != "":
		_start_spawning(true)
...
func set_pool():
	...
	Spawning.create_pool(
		props, shared_area_name, pool_amount, !Spawning.bullet(props).has("anim_idle")
	)
...
func _physics_process(delta):
	...
	if (
		...
		and (
			global_position.distance_to(get_node(auto_distance_from).global_position)
			<= auto_start_at_distance
		)
	):
		active = true
	...
	if can_respawn and auto_call and active and auto_pattern_id != "":
		...
		can_respawn = false
		if not rotating_speed > 0:
			set_physics_process(false)


func on_screen(is_on):
	_start_spawning(is_on)
...
func checkTrigger():
	if not (active and auto_pattern_id != "" and trig_container):
		return
...
func _get_property_list() -> Array:
	return [
		{name = "active", type = TYPE_BOOL, usage = PROPERTY_USAGE_DEFAULT},
		{name = "auto_pattern_id", type = TYPE_STRING, usage = PROPERTY_USAGE_DEFAULT},
		{name = "shared_area_name", type = TYPE_STRING, usage = PROPERTY_USAGE_DEFAULT},
		{name = "rotating_speed", type = TYPE_FLOAT, usage = PROPERTY_USAGE_DEFAULT},
		{name = "pool_amount", type = TYPE_INT, usage = PROPERTY_USAGE_DEFAULT},
		{
		...
		{name = "auto_start_on_cam", type = TYPE_BOOL, usage = PROPERTY_USAGE_DEFAULT},
		{name = "auto_start_after_time", type = TYPE_FLOAT, usage = PROPERTY_USAGE_DEFAULT},
		{name = "auto_start_at_distance", type = TYPE_FLOAT, usage = PROPERTY_USAGE_DEFAULT},
		{name = "auto_distance_from", type = TYPE_NODE_PATH, usage = PROPERTY_USAGE_DEFAULT},
		{
		...
		{name = "trigger_container", type = TYPE_NODE_PATH, usage = PROPERTY_USAGE_DEFAULT}
		#		,{
		#			name = "Random",
		#			type = TYPE_NIL,
		#			hint_string = "r_",
...
###################################
## extensions by TinyTakinTeller ##
###################################


func _start_spawning(auto = true) -> void:
